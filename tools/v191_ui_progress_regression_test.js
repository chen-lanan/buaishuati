...
assert.strictEqual(read('VERSION.txt').trim(), '1.9.1');
assert.strictEqual(read('miniapp-source/VERSION.txt').trim(), '1.9.1');
assert.ok(/versionCode 191/.test(read('app/build.gradle')));
assert.ok(/versionName '1\.9\.1'/.test(read('app/build.gradle')));
...
assert.ok(practiceWxml.includes('transform: scaleX({{progressRatio}})'));
assert.ok(practiceCss.includes('transform-origin: left center'));
assert.ok(practiceCss.includes('width: 100%; height: 100%'));
assert.ok(practiceJs.includes('session.index >= totalQuestions - 1'));
assert.ok(practiceJs.includes('? 1'));
assert.ok(!practiceWxml.includes('progressPercent'));
...
console.log('v1.9.1 header, abnormal pulse, type filter and donation removal regression: PASS');
